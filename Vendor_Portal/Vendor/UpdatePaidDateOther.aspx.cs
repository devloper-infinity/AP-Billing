...
{
    public partial class UpdatePaidDateOther : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string GetAllInvoiceForUpdatePaidDateAsPerPayees(int InvoiceID, string InvType)
        {
            DataTable dt1 = new bllInvoice().GetAllInvoiceForUpdatePaidDateAsPerPayees(InvoiceID, InvType);
        ...
        [WebMethod]
        public static string GetInfinity_InvoiceDetails_Approval_Other(string InvoiceType, string Company)
        {
            DataTable dt1 = new bllInvoice().GetInfinity_InvoiceDetails_Approval_Other(int.Parse(HttpContext.Current.User.Identity.Name.ToString()), Company, InvoiceType);
            List<Dictionary<string, object>> rows = new List<Dictionary<string, object>>();
            Dictionary<string, object> row;
            if (dt1 != null)
            {
        ...
        [WebMethod]
        public static int UpdatePaidDateOthers(string PayeeIDs, string InvoiceID, string Company, string Vendor, string Remark, string PaymentDate, string UTR, string TotalAmt)
        {
            int ReturnValue = 0;
            try
            {
                string str_split = PayeeIDs.Substring(2);
                string[] IDs = str_split.Split(',');

                string UTRNo_split = UTR.Substring(10);
                string[] UTRNo = UTRNo_split.Split('|');

                string TotalAmt_split = TotalAmt.Substring(10);
                string[] Amt = TotalAmt_split.Split('|');
                ...
                foreach (var sub_str in IDs)
                {
                    ...
                    htParam.Add("PayeeID", sub_str);
                    htParam.Add("InvoiceID", InvoiceID);
                    htParam.Add("PaymentDate", PaymentDate);
                    htParam.Add("Remark", Remark);
                    htParam.Add("UTR", UTRNo[i]);
                    htParam.Add("TotalAmount", Amt[i]);
                    htParam.Add("Company", Company);
                    htParam.Add("Vendor", Vendor);
                    htParam.Add("AddedBy", int.Parse(HttpContext.Current.User.Identity.Name.ToString()));

                    ReturnValue = new bllInvoice().UpdatePaidDateOthers(htParam);

                    if (ReturnValue > 0)
                    {
                        SendEmail_Body_UpdateDate(Convert.ToInt32(sub_str), Company,Convert.ToInt32(InvoiceID));

                        //SendEmail_Body_UpdateDate(Convert.ToInt32(39), "IPS", Convert.ToInt32(534));
                    }
                }
            }
            catch (Exception ex)
        ...
        [WebMethod]
        public static void SendEmail_Body_UpdateDate(int PayeeID, string Company,int InvoiceId)
        {
            try
            {
                ...
                if (Company == "IPS")
                {
                    ...
                    dt = new bllInvoice().GetAllInvoiceForUpdatePaidDateOtherPayee(PayeeID, InvoiceId);
                }
                else if (Company == "Canopy")
                {
                    PaymentFrom = "Canopy";
                    dt = new bllInvoice().GetInvoiceDetailsForUpdateDateByOtherPayee_Canopy(PayeeID, InvoiceId);
                }
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    htmlBody.Append("<tr><td>Payment To :</td><td>" + Convert.ToString(dt.Rows[0]["InvoiceType"]) + " - " + Convert.ToString(dt.Rows[0]["PayTo"]) + "</td></tr>");
                    htmlBody.Append("<tr><td>Payment Method :</td><td>ACH</td></tr>");
                    htmlBody.Append("<tr><td>Payment Amount :</td><td>" + Convert.ToString(dt.Rows[0]["TotalLoanAmount"]) + "</td></tr>");
                    htmlBody.Append("<tr><td>Payment Date :</td><td>" + Convert.ToString(dt.Rows[0]["PaymentDate"]) + "</td></tr>");
                    ...
                    htmlBody.Append("<table width=\"500px\" ><tr><td align=\"left\"><br />Infinity IPS. has initiated payment for the listed invoices and is scheduled to credit the bank <br /> <b>account ending in " + Convert.ToString(dt.Rows[0]["AccNo"]) + " within two working days.<b></td></tr><tr>");
