...
{
    public partial class AddInvoiceKCB : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (file.FileName.Contains("RemoteUW"))
                //{
                //    NewFileName_RemoteInvoice = Server.MapPath("..//TempFiles//" + "Invoice_" + file_Name);
                //    FileStream objfilestream = new FileStream(NewFileName_RemoteInvoice, FileMode.Create, FileAccess.ReadWrite);
                //    objfilestream.Write(binaryWriteArray, 0,
        ...
        [WebMethod]
        public static int InsertRemoteUWKCBInvoice(string Month, string Year, string InvoiceDate, string DueDate, string LoanCount, string InvoiceAmount, string InvoiceNumber)
        {
            var mc_KCB = new AddInvoiceKCB();
            int returnvalue = 0;
            //Convert.ToDateTime(InvoiceDate).ToString("MMM")
            string InvoiceType = "KCB";
            ...
            htParam.Add("Remark", "KCB" + '~' + Month + '-' + Year);
            returnvalue = new bllInvoice().Insert_Infinity_InvoiceDetails(htParam);
            if (returnvalue > 0)
            {
                mc_KCB.sendEmailAddNewInvoice_KCB(returnvalue,"KCB");
            }
            return returnvalue;
        }
        ...
        public static int InsertRemoteUWKCBExcel(int InvoiceID, string Month, string Year)
        {
            int returnvalue = 0;
        ...
        [WebMethod]
        public static string VerifyRemoteUWKCB(int InvoiceID, string Month, string Year)
        {
            DataTable dt1 = new bllInvoice().GetIPSRemoteUWKCBForVerify(InvoiceID, Month, Year);
        ...
        [WebMethod]
        public void sendEmailAddNewInvoice_KCB(int InvoiceId, string strInvoiceType)
        {
            var mc_KCB = new AddInvoiceKCB();
            ...
            else if (strInvoiceType == "Abstractor")
            {
                ToAddress = "n.prasad@infinityinternationals.us";
                ToCC = "";
            }

            else if (strInvoiceType == "Remote UW")
            {
                ToAddress = "k.manoj@infinity-data.com,Robert.Williams@infinity-data.com";
                ToCC = "s.chandrakant@infinity-data.com";
            }

            else if (strInvoiceType == "KCB" || strInvoiceType == "PACER")
            {
                ToAddress = "k.manoj@infinity-data.com,t.philemon@infinityinternationals.us";
                ToCC = "s.chandrakant@infinity-data.com";
            }
            else
            {
                ToAddress = "k.manoj@infinity-data.com,g.princeton@infinityinternationals.us";
                ToCC = "s.chandrakant@infinity-data.com";
            }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                mc_KCB.sendEmailInvoiceApproval_IPS(ToAddress, ToCC, "n.nilkanth@infinityinternationals.us,p.kedar@infinityinternationals.us,b.shubhangi@infinityinternationals.us", Subject, Path, htmlBody);
            }
